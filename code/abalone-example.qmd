...
H~0~: **[pH does not predict abolone growth (change in shell surface areas per day, mm^2^ d^-1^)]**  
...
# creating a new object called abolone_clean from the og dataset
...
```{r abalone-model}
abalone_model <- lm(
  change_in_area ~ mean_ph, # formula in shell surface area per day as a function of pH
...
Residuals seem homoscedastic (consistent across values) based on Residuals vs Fitted and Scale-Location plots. Residuals also seem normally distributed based on QQ plot. No major outliers based on Residuals vs Leverage

We evaluated model assumptions visually. For homoscedasticity of residuals we used Residual vs Fitted and Scale Location plots. For normality of residuals we used QQ plots. Based on our diagnostics, there were no major deviations from homoscedasticity ot normality of residuals. There were also no outliers that might influence model estimates or predictions. 
...
**-18.5 +- 6.16 (SE). WHen pH = 0 the model predicst that abalone change in shell surface area per day is -18.5 +- 6.16. (Note to seld: this is not necessarily biologically realistic, but is something the model generates as an estimate).**
...
**x: predicter variable values (ph)**  

**predicted: model predictions fro change in shell surface area per day**

**std.error: precision of the prediction**

**conf.low and conf.high: range for the 95% CI (I around model predictions)**
...
**at ph=8 th emodel predicst that abolone change in shell surface area per day should be 5.36 mm^2^ d^-1^ (95% CI: [5.08, 5.64])
